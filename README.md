...
```bash
custom-spotify-blend$ sam build --use-container
```

The SAM CLI installs dependencies defined in `spotify_blend/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
custom-spotify-blend$ sam local invoke SpotifyBlendFunction --event events/event.json
```
...
```bash
custom-spotify-blend$ sam logs -n SpotifyBlendFunction --stack-name custom-spotify-blend --tail
```
...
```bash
aws cloudformation delete-stack --stack-name custom-spotify-blend
```
